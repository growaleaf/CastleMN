...
// The zCSTL block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCSTL is a hash of the serial
...
bool CZCstlStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    ...
    if (!pwallet->CreateZCSTLOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCSTL output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCSTL", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCSTLOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCSTL output", __func__);
