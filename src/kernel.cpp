...
bool GetHashProofOfStake(const CBlockIndex* pindexPrev, CStakeInput* stake, const unsigned int nTimeTx, const bool fVerify, uint256& hashProofOfStakeRet) {
    ...
    if (fVerify) {
        LogPrint("staking", "%s : nStakeModifier=%s (nStakeModifierHeight=%s)\n"
                "nTimeBlockFrom=%d\nssUniqueIDD=%s\n-->DATA=%s",
            __func__, HexStr(modifier_ss), ((stake->IsZCSTL()) ? "Not available" : std::to_string(stake->getStakeModifierHeight())),
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    ...
    if (CZCstlStake* zCSTL = dynamic_cast<CZCstlStake*>(stake)) {
        ...
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zCSTL stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zCSTL fails context checks", __func__);
    } else {
        ...
        CCstlStake* cstlInput = new CCstlStake();
        cstlInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(cstlInput);
    }
    return true;
}
...
bool CheckProofOfStake(const CBlock& block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (!CheckStakeKernelHash(pindexPrev, block.nBits, stake.get(), nTxTime, hashProofOfStake, true) && pindexfrom->nHeight > 4000)
