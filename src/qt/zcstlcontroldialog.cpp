...
bool CZCstlControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZCstlControlDialog::COLUMN_DENOMINATION || column == ZCstlControlDialog::COLUMN_VERSION || column == ZCstlControlDialog::COLUMN_CONFIRMATIONS)
    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zCSTL Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZCstl->setProperty("cssClass", "text-main-purple");
    ui->labelZCstl_int->setProperty("cssClass", "text-main-purple");
...
void ZCstlControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZCstlControlWidgetItem* itemDenom(new CZCstlControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZCstlControlWidgetItem *itemMint = new CZCstlControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zCSTL.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCSTL seed used to mint this zCSTL is not the same as currently hold in the wallet";
            else
