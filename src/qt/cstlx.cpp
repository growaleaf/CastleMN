...
#include "qt/castle/splash.h"
#include "qt/castle/welcomecontentwidget.h"
...
public:
    ...
    /// Get window identifier of QMainWindow (CASTLEGUI)
...
{
    window = new CASTLEGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &CASTLEGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &CASTLEGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(CASTLEGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(CASTLEGUI::DEFAULT_WALLET);
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &CASTLEGUI::handlePaymentRequest);
        connect(window, &CASTLEGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. CASTLE can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
