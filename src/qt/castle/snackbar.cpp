...
#include "qt/castle/forms/ui_snackbar.h"
#include "qt/castle/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(CASTLEGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &CASTLEGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
