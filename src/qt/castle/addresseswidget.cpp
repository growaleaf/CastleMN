...
#include "qt/castle/forms/ui_addresseswidget.h"
#include "qt/castle/addresslabelrow.h"
#include "qt/castle/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(CASTLEGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter CASTLE address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress cstlAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(cstlAdd.Get(), label.toUtf8().constData(),
                cstlAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
