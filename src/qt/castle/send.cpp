...
#include "qt/castle/send.h"
#include "qt/castle/forms/ui_send.h"
#include "qt/castle/addnewcontactdialog.h"
#include "qt/castle/qtutils.h"
#include "qt/castle/sendchangeaddressdialog.h"
#include "qt/castle/optionbutton.h"
#include "qt/castle/sendconfirmdialog.h"
#include "qt/castle/myaddressrow.h"
#include "qt/castle/guitransactionsutils.h"
...
SendWidget::SendWidget(CASTLEGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("CSTL");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zCSTL");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (CSTL) or private coins (zCSTL)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("CASTLE address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 CSTL");
    ...
    setCssProperty(coinIcon, "coin-icon-cstl");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onCSTLSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onCSTLSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send CSTL");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zCSTL");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZcstl = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZcstl));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZcstl ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendCstl = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendCstl ? AskPassphraseDialog::Context::Send_CSTL : AskPassphraseDialog::Context::Send_zCSTL, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendCstl) ? send(recipients) : sendZcstl(recipients)) {
...
bool SendWidget::sendZcstl(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zCSTL is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZcstl(
        ...
        inform(tr("zCSTL transaction sent!"));
        ZCstlControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZCSTL_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zCSTL require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zCSTL transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isCSTL){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zCstlControl->exec();
            ui->btnCoinControl->setActive(!ZCstlControlDialog::setSelectedMints.empty());
            zCstlControl->deleteLater();
        } else {
            inform(tr("You don't have any zCSTL in your balance to select."));
        }
    }
}
...
void SendWidget::onCSTLSelected(bool _isCSTL){
    isCSTL = _isCSTL;
    setCssProperty(coinIcon, _isCSTL ? "coin-icon-cstl" : "coin-icon-zcstl");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress cstlAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(cstlAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(cstlAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
