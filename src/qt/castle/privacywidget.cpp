...
#include "qt/castle/privacywidget.h"
#include "qt/castle/forms/ui_privacywidget.h"
#include "qt/castle/qtutils.h"
#include "guiutil.h"
#include "qt/castle/denomgenerationdialog.h"
#include "qt/castle/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(CASTLEGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zCSTL anonymizes your CSTL by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zCSTL or convert back to CSTL"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 CSTL ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zCSTL");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zCSTL");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zCSTL");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zCSTL");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zCSTL");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zCSTL");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zCSTL");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zCSTL");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zCSTL Movements"));
    ...
    // Only Convert to CSTL enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzCSTL->setTitleClassAndText("btn-title-grey", "Total 0 zCSTL");
    ui->btnTotalzCSTL->setSubTitleClassAndText("text-subtitle", "Show denominations of zCSTL owned.");
    ui->btnTotalzCSTL->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select CSTL outputs to mint into zCSTL.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zCSTL");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzCSTL, SIGNAL(clicked()), this, SLOT(onTotalZcstlClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zCSTL");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of CSTL to mint into zCSTL"));
    }else{
        btnText = tr("Convert back to CSTL");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zCSTL to convert back into CSTL"));
    }
...
void PrivacyWidget::onTotalZcstlClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzCSTL->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzCSTL->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zCSTL is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zCSTL, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zCSTL").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zCSTL minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZcstl(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zCSTL converted back to CSTL"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any CSTL to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzCSTL->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
