    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of CSTL staked and/or generated from Masternodes."));
    ...
    ui->labelAmountMNRewards->setText("0 CSTL");
    ui->labelAmountCstl->setText("0 CSTL");
    ...
    ui->btnHowTo->setText(tr("How to get CSTL"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isCstl = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZCSTL && modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        int max = std::max(cstl, mnrewards);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("CSTL");
    set1 = new QBarSet("CSTL MN");
    ...
    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCstl > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountCstl, "text-stake-cstl");
        setCssProperty(ui->labelAmountMNRewards, "text-mnrewards-mnrcstl");
    } else {
        setCssProperty(ui->labelAmountCstl, "text-stake-cstl-disable");
        setCssProperty(ui->labelAmountMNRewards, "text-stake-mnrewards-disable");
    }
    forceUpdateStyle({ui->labelAmountCstl, ui->labelAmountMNRewards});
