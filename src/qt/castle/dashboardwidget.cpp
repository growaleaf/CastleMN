...
#include "qt/castle/dashboardwidget.h"
#include "qt/castle/forms/ui_dashboardwidget.h"
#include "qt/castle/sendconfirmdialog.h"
#include "qt/castle/txrow.h"
...
DashboardWidget::DashboardWidget(CASTLEGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareCstl, "square-chart-cstl");
    setCssProperty(ui->labelSquarezCstl, "square-chart-zcstl");
    setCssProperty(ui->labelCstl, "text-chart-cstl");
    setCssProperty(ui->labelZcstl, "text-chart-zcstl");
    ...
    setCssProperty(ui->labelAmountCstl, "text-stake-cstl-disable");
    setCssProperty(ui->labelAmountZcstl, "text-stake-zcstl-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &CASTLEGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &CASTLEGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &CASTLEGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isCstl = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZCSTL;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair CSTL, zCSTL
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            cstl = (pair.first != 0) ? pair.first / 100000000 : 0;
            zcstl = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalCstl += pair.first;
            chartData->totalZcstl += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesCstl.append(cstl);
        chartData->valueszCstl.append(zcstl);

        int max = std::max(cstl, zcstl);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesCstl);
    set1->append(chartData->valueszCstl);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCstl > 0 || chartData->totalZcstl > 0) {
        setCssProperty(ui->labelAmountCstl, "text-stake-cstl");
        setCssProperty(ui->labelAmountZcstl, "text-stake-zcstl");
    } else {
        setCssProperty(ui->labelAmountCstl, "text-stake-cstl-disable");
        setCssProperty(ui->labelAmountZcstl, "text-stake-zcstl-disable");
    }
    forceUpdateStyle({ui->labelAmountCstl, ui->labelAmountZcstl});
    ui->labelAmountCstl->setText(GUIUtil::formatBalance(chartData->totalCstl, nDisplayUnit));
    ui->labelAmountZcstl->setText(GUIUtil::formatBalance(chartData->totalZcstl, nDisplayUnit, true));
