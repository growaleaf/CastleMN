...
#include "qt/castle/topbar.h"
#include "qt/castle/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/castle/lockunlock.h"
#include "qt/castle/qtutils.h"
#include "qt/castle/receivedialog.h"
...
TopBar::TopBar(CASTLEGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopCstl, ui->labelAmountTopzCstl}, "amount-small-topbar");
    setCssProperty({ui->labelAmountCstl, ui->labelAmountzCstl}, "amount-topbar");
    setCssProperty({ui->labelPendingCstl, ui->labelPendingzCstl, ui->labelImmatureCstl, ui->labelImmaturezCstl}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelAmountTopzCstl->setVisible(false);
    ui->labelAmountzCstl->setVisible(false);
    ui->labelPendingzCstl->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("CASTLE")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // CSTL Balance
    CAmount cstlAvailableBalance = balance;
    // zCSTL Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalCstl = GUIUtil::formatBalance(cstlAvailableBalance, nDisplayUnit);
    //QString totalzCstl = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopCstl->setText(totalCstl);
    //ui->labelAmountTopzCstl->setText(totalzCstl);

    // Expanded
    ui->labelAmountCstl->setText(totalCstl);
    //ui->labelAmountzCstl->setText(totalzCstl);

    ui->labelPendingCstl->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    //ui->labelPendingzCstl->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureCstl->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    //ui->labelImmaturezCstl->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
