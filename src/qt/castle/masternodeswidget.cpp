...
#include "qt/castle/forms/ui_masternodeswidget.h"
#include "qt/castle/qtutils.h"
#include "qt/castle/mnrow.h"
#include "qt/castle/mninfodialog.h"

#include "qt/castle/masternodewizarddialog.h"
...
MasterNodesWidget::MasterNodesWidget(CASTLEGUI *parent) :
    PWidget(parent),
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= (COIN * walletModel->getRequiredMasternodeCollateral())){
            inform(tr("Not enough balance to create a masternode, 40,000 CSTL required."));
            return;
        }
        showHideOp(true);
