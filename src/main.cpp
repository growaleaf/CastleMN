...
#include "zcstl/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zCSTL spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcspend or zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcstl");
                }
                ...
                if (isPublicSpend) {
                    ...
                    if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                        return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                                __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zcstl");
                    }
            ...
            // Check that zCSTL mints (if included) are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCSTLStake)
{
    int64_t ret = 0;
    ...
    if (nHeight >= 160000)
        ret = blockValue * 68 / 100; //68% of the block reward
...
void RecalculateZCSTLMinted()
{
    ...
    uiInterface.ShowProgress(_("Recalculating minted ZCSTL..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZCSTL..."), percent);
        }
...
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZCSTL..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZCSTL..."), percent);
        }
...
bool UpdateZCSTLSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZCSTLSupply(block, pindex, fJustCheck))
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 CSTL.
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired && (pindexPrev->nHeight > 100000)) {
        // Castle Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().CastleBadBlockTime()) && (block.nBits == (uint32_t) Params().CastleBadBlocknBits())) {
            // accept CASTLE block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> cstlInputs;
        std::vector<CTxIn> zCSTLInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zCSTLInputs.push_back(stakeIn);
            }else{
                cstlInputs.push_back(stakeIn);
            }
        }
        const bool hasCSTLInputs = !cstlInputs.empty();
        const bool hasZCSTLInputs = !zCSTLInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasCSTLInputs) {
                    ...
                    for (const CTxIn& cstlIn : cstlInputs)
                        if(cstlIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if(hasZCSTLInputs && in.IsZerocoinSpend())
            ...
            // Now that this loop if completed. Check if we have zCSTL inputs.
            if(hasZCSTLInputs) {
                for (const CTxIn& zCstlInput : zCSTLInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zCstlInput);
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zcstl", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zcstl", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
