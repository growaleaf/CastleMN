...
// Copyright (c) 2015-2020 The CASTLE developers
...
/** Fees smaller than this (in ucstl) are considered zero fee (for relaying, mining and transaction creation)
        ...
        bool* pfMissingInputs, bool fOverrideMempoolLimit, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zCSTL mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zCSTL mint output in tx %s",
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    //Track zCSTL money supply
    if (!UpdateZCSTLSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zCSTL supply", __func__);
...
{
    util::ThreadRename("castle-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
   ...
   if (pindex->nHeight > 2000 && isPoSActive && block.IsProofOfWork())
    ...
    // Update zCSTL money supply map
    if (!UpdateZCSTLSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zCSTL supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zCSTL supply was off (due to serial duplication off-chain on main net)
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateCSTLSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nCastleBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nCastleBadBlockBits)) {
