...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, castle, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all CSTL specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-castlestake=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-castlestake=<n>", strprintf(_("Enable or disable staking functionality for CSTL inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcastlestake=<n>", strprintf(_("Enable or disable staking functionality for zCSTL inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzpiv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCSTL minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpivbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zCSTL backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
/** Sanity checks
 *  Ensure that CSTL is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (fDisableWallet) {
#endif
        if (SoftSetBoolArg("-castlestake", false))
            LogPrintf("AppInit2 : parameter interaction: wallet functionality not enabled -> setting -castlestake=0\n");
#ifdef ENABLE_WALLET
    }
#endif
...
#endif
    // Make sure only a single CSTL process is using the data directory.
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //CSTL: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //CSTL specific: zerocoin and spork DB's
                ...
                // CSTL: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // CSTL: recalculate Accumulator Checkpoints that failed to database properly
...
//    if (nAnonymizeCastleAmount < 2) nAnonymizeCastleAmount = 2;
    ...
    LogPrintf("Anonymize CSTL Amount %d\n", nAnonymizeCastleAmount);
