...
#include "zcstl/accumulatorcheckpoints.h"
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZcstlThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 35801, 51474));
    ...
    std::string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, castle, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-cstlstake=<n>", strprintf(_("Enable or disable staking functionality for CSTL inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zcstlstake=<n>", strprintf(_("Enable or disable staking functionality for zCSTL inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zcstlbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zCSTL backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 35802, 51475));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //CASTLE: Load Accumulator Checkpoints according to network (main/test/regtest)
        ...
        do {
            ...
            try {
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zcstlSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();

                    if (pblockindex->GetZerocoinSupply() < zcstlSupplyCheckpoint) {
                        // Trigger reindex due wrapping serials
                        LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zcstlSupplyCheckpoint/COIN);
                        reindexDueWrappedSerials = true;
                    } else if (pblockindex->GetZerocoinSupply() > zcstlSupplyCheckpoint) {
                        // Trigger global zCSTL reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateCSTLSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
               ...
               ///     CAmount zcstlSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
               ///     if (pblockindex->GetZerocoinSupply() != zcstlSupplyCheckpoint)
               //         return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex-//>GetZerocoinSupply()/COIN , zcstlSupplyCheckpoint/COIN));
                //}
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzCSTLWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zCSTLWallet
        uiInterface.InitMessage(_("Syncing zCSTL wallet..."));
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZcstlThread(threadGroup);
    }
