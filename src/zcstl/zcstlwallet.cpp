...
CzCSTLWallet::CzCSTLWallet(CWallet* parent)
{
    ...
    //Check for old db version of storing zcstl seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCSTLSeed_deprecated(seed)) {
            ...
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCSTLSeed_deprecated()) {
                    ...
                    LogPrintf("%s: failed to remove old zcstl seed\n", __func__);
                }
            }
        }
    }
...
bool CzCSTLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    else if (!walletdb.ReadZCSTLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCSTLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
void CzCSTLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCSTLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCSTLWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzCSTLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCSTL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
uint512 CzCSTLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCSTLWallet::GenerateDeterministicZCSTL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCSTLWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCSTLWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCSTLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
