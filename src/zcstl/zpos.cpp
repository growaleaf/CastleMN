...
bool CLegacyZCstlStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZCstlStake from non zc-spend", __func__);
    ...
    *this = CLegacyZCstlStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zcstl stake origin", __func__);

    // All good
    return true;
}

CLegacyZCstlStake::CLegacyZCstlStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZCstlStake::GetIndexFrom()
{
...
CAmount CLegacyZCstlStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZCstlStake::GetUniqueness() const
{
...
bool CLegacyZCstlStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zCSTL stake block: height %d outside range", __func__, nHeight);
