...
CzCSTLTracker::CzCSTLTracker(std::string strWalletFile)
{
...
CzCSTLTracker::~CzCSTLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCSTLTracker::Init()
{
...
bool CzCSTLTracker::Archive(CMintMeta& meta)
{
...
bool CzCSTLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCSTLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCSTLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCSTLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCSTLTracker::GetSerialHashes()
{
...
CAmount CzCSTLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCSTLTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCSTLTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCSTLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCSTLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCSTLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCSTLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCSTLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCSTLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCSTLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCSTLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCSTLWallet* zCSTLWallet)
{
    bool iszCSTLWalletInitialized = (NULL != zCSTLWallet);
    CMintMeta meta;
    ...
    if (! iszCSTLWalletInitialized)
        zCSTLWallet = new CzCSTLWallet(strWalletFile);
    meta.isSeedCorrect = zCSTLWallet->CheckSeed(dMint);
    if (! iszCSTLWalletInitialized)
        delete zCSTLWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCSTLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCSTLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCSTLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCSTLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCSTLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCSTLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCSTLWallet* zCSTLWallet = new CzCSTLWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCSTLWallet);
        }
        delete zCSTLWallet;
        LogPrint("zero", "%s: added %d dzcstl from DB\n", __func__, listDeterministicDB.size());
    }
