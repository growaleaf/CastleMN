    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZCSTLSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZCSTLSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zcstlseed to wallet";
    }
    //the use case for this is no password set seed, mint dzCSTL,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZCSTLSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZCSTLSeed(hashSeed, vchSeed)) {
...
void CWallet::doZCstlRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZCSTL)
{
    //If this is a zapwallettx, need to read zcstl
    if (fCheckZCSTL && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
        ...
        CTxDestination * changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zCSTL public spend transaction
...
{
    zwalletMain = zwallet;
    zcstlTracker = std::unique_ptr<CzCSTLTracker>(new CzCSTLTracker(this));
}
