...
#include "zcstl/zcstlwallet.h"
#include "zcstl/zcstltracker.h"
...
{
    return zcstlTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZCSTL = GetBoolArg("-zapwallettxes", false);
    if (fCheckZCSTL)
        zcstlTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zcstl
            if (fCheckZCSTL && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zCSTL in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zcstlTracker->GetMints(true);
    ...
    return zcstlTracker->GetBalance(false, false);
}
...
{
    return zcstlTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zcstlTracker->ListMints(true, true, true);
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZcstlBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 CSTL collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 CSTL collateral
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        //Masternode payment
        FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZCSTL());
    ...
    if (!txNew.vin[0].scriptSig.IsZerocoinSpend()) {
        ...
        for (const CTxOut& out : txNew.vout) {
            ...
            if (!zcstlTracker->HasPubcoinHash(hashPubcoin))
                return error("%s: could not find pubcoinhash %s in tracker", __func__, hashPubcoin.GetHex());

            CMintMeta meta = zcstlTracker->GetMetaFromPubcoin(hashPubcoin);
...
bool CWallet::CreateZCSTLOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZCSTL(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZCSTLOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zcstl output", __func__);
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        ...
        if(!zcstlTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zcstlTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zcstlTracker->UpdateState(meta))
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GenerateAccumulatorWitness(&coinWitness, mapAccumulators, pindexCheckpoint)) {
            receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                              ZCSTL_FAILED_ACCUMULATOR_INITIALIZATION);
        ...
        if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            ...
            if (!mint.GetKeyPair(key))
                return error("%s: failed to set zCSTL privkey mint version=%d", __func__, nVersion);
        ...
        try {
            ...
            if (!CheckCoinSpend(spend, accumulator, receipt)) {
                receipt.SetStatus(_("CoinSpend: failed check"), ZCSTL_SPEND_ERROR);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZCSTL_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZCSTL_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZCSTL_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZCSTLModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZCSTL_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zCSTL"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zCSTL public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zCSTL used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zcstlTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zcstlTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zcstlTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zcstlTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zcstlTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fzcstlAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzcstlAuto ? "-autozcstlbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
...
void CWallet::ZCstlBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozcstlbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zcstlbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zcstlbackuppath", ""));
...
)
{
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zcstlTracker->SetPubcoinNotUsed(hashPubcoin);
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zcstlTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zcstlTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
...
{
    CMintMeta meta;
    if (!zcstlTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zcstlTracker->HasPubcoin(bnValue))
        return true;
...
void CWallet::setZWallet(CzCSTLWallet* zwallet)
{
    zwalletMain = zwallet;
    zcstlTracker = std::unique_ptr<CzCSTLTracker>(new CzCSTLTracker(strWalletFile));
}

CzCSTLWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
void CWallet::setZCstlAutoBackups(bool fEnabled)
{
