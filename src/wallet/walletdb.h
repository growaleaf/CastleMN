...
// Copyright (c) 2016-2020 The CASTLE developers
...
public:
    ...
    bool WriteZCSTLSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZCSTLSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZCSTLSeed_deprecated(uint256& seed);
    bool EraseZCSTLSeed();
    bool EraseZCSTLSeed_deprecated();

    bool WriteZCSTLCount(const uint32_t& nCount);
    bool ReadZCSTLCount(uint32_t& nCount);
