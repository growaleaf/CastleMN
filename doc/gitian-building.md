...
*Setup instructions for a gitian build of CSTL using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CSTL
...
Clone the git repositories for castle and gitian and then checkout the castle version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/castle-crypto/castle.git
cd castle
...
Follow the instructions in [doc/release-process.md](release-process.md) in the castle repository
...
To build CSTL (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the castle repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/castle/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/castle-crypto/castle
...
For example:
```bash
URL=https://github.com/crowning-/castle.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/castle-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/castle-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/castle-build.assert
```
...
[castle/gitian.sigs](https://github.com/castle-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
