...
to the CASTLE GitHub release page.
...
- [Building CASTLE Core](#building-castle-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/castle-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/castle-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/castle-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/castle-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/castle-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/castle-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/castle-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@castle.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/castle-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/castle-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/castle-osx-*-build.assert
```
