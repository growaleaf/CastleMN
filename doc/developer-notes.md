...
- [Development guidelines](#development-guidelines)
    - [General CASTLE Core](#general-castle-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
CASTLE Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
CASTLE Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/castle-project/castle/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_castle
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_castle --log_level=test_suite
$ valgrind -v --leak-check=full src/castled -printtoconsole
```
...
# A coverage report will now be accessible at `./test_castle.coverage/index.html`.
```
...
- BitcoinMiner : Generates CSTLs (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against CASTLE Core so that they can be integrated
...
file descriptors it will cause problems with CASTLE's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:CASTLE-Project/CASTLE.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `castle-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `castle-cli` and the GUI debug console use this table to determine how to
